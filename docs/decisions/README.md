...
Scope is the reach of the decision: process constrains every change, architecture
constrains every module, module constrains a single module. The number is an
immutable identifier, not a ranking.
...
Copy `_template/ADR-template.md`, assign the next sequential number, name
the file `ADR-XXX-short-english-title.md`, and add the corresponding row to the table of the
group it belongs to.

## Format

**MADR minimal**, following Michael Nygard's original template as extended by MADR 4.0
(Context and Problem Statement, Decision Drivers, Considered Options, Decision Outcome,
Consequences), plus a lightweight YAML front-matter (`status`, `date`, `decision-makers`)
inspired by YADR.
